...
void parseAndUpdateData(const std::string &jsonString, struct_message &data) {
  ...
  DynamicJsonDocument jsonDoc(512); // Increased size for larger payloads
  ...
  if (!error) {
    ...
    if (jsonDoc.containsKey("animationdelay")) {
      ...
      Serial.print(F("Animation delay: "));
      Serial.println(data.animationdelay);
    }
    
    // Apply changes immediately
    if (brightnessChanged) {
      strip.setBrightness(data.brightness);
      Serial.print(F("Brightness applied: "));
      Serial.println(data.brightness);
    }
    
    // Force immediate update for certain modes
    if (colorChanged && strcmp(data.lightmode, "setColor") == 0) {
      Serial.println(F("Forcing immediate color update"));
      setColor();
    }
    
    Serial.println(F("Data update complete"));
  } else {
...
void setup() {
  ...
  Serial.println(F("Initializing strip with test pattern"));
  for(int i = 0; i < min((uint16_t)10, strip.numPixels()); i++) {
  ...
  Serial.print(F("Strip initialized with "));
...
void handleStrip() {
  ...
  if (millis() - lastDebug > 5000) {
    ...
    Serial.println(myData.animationdelay);
    lastDebug = millis();
  }
...
void loop() {
  ...
  // Check touch sensor
  if (touchRead(4) < 20) {
    Serial.println(F("Touch triggered - setting test color"));
    for(int i = 0; i < min((uint16_t)5, strip.numPixels()); i++) {
  ...
  if (currentMillis - lastConnCheck > 10000) {
