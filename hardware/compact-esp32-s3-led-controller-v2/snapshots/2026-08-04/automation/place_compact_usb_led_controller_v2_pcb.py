...
# EasyEDA source units are 10 mil (0.254 mm). The one-side-assembly redesign is
# 19 x 23 mm: x=4018.0315..4092.8346, y=3425..3515.5512. U_ESP is the only
# top-side SMT component and is intentionally left for user reflow; every other
# SMT part is bottom-side for one-side JLC assembly. Power input remains right,
# LED output remains left, and the MINI-1 RF geometry stays fixed.
POSITIONS={
 'U_ESP':(4055.433,3455.5),'J_LED':(4041.6535,3507.6772),
 'J_5V_IN':(4075.1181,3507.6772),
 # Bottom-side local clusters sit under the non-antenna module region. The
 # protected-input parts occupy the lower-right corridor near the input row.
 'D_VBUS':(4089.0,3496.5),'F_VBUS':(4078.0,3483.0),
 'U_3V3':(4034.0,3459.0),'C_VBUS_IN':(4026.0,3472.0),
 'C_3V3_OUT1':(4043.0,3469.0),'C_ESP_DEC':(4031.5,3438.767),
 'R_BOOT':(4031.5,3445.5),'R_EN':(4077.5,3436.5),
 'C_EN':(4087.0,3438.5),'U_LEVEL':(4058.5,3478.0),
 'C_LEVEL':(4074.0,3470.0),'R_LEVEL_PD':(4048.0,3478.5),
 # Bottom-side LED resistor aligns vertically above the LED cable row.
 'R_DATA':(4051.0,3494.5),
 # Height-efficient underside 3 x 2 field: 2.00-mm column pitch, 3.00-mm row
 # pitch, shifted left to keep the protected-input corridor unobstructed.
 'TP_BOOT':(4025.5000,3484.0000),'TP_GND':(4033.3740,3484.0000),'TP_RX':(4041.2480,3484.0000),
 'TP_5V':(4025.5000,3495.8110),'TP_TX':(4033.3740,3495.8110),'TP_EN':(4041.2480,3495.8110),
}
TPS={'TP_5V','TP_GND','TP_TX','TP_RX','TP_EN','TP_BOOT'}
BOTTOM_PARTS={'C_ESP_DEC','R_BOOT','R_EN','C_EN','U_3V3','C_VBUS_IN','C_3V3_OUT1','U_LEVEL','C_LEVEL','R_LEVEL_PD','D_VBUS','F_VBUS','R_DATA'}
# EasyEDA records rotate-right as a 90-degree decrement.
ROTATIONS={'D_VBUS':270,'U_3V3':270,'F_VBUS':180,'R_BOOT':180,'R_LEVEL_PD':270,'R_DATA':270,'C_EN':270,'C_LEVEL':0,'C_VBUS_IN':180,'C_3V3_OUT1':0,'U_LEVEL':270}
...
def main():
 ...
 try:
  ...
  redesign_chamfered=[{'x':4023.9375,'y':3425},{'x':4092.8346,'y':3425},{'x':4092.8346,'y':3519.488},{'x':4018.0315,'y':3519.488},{'x':4018.0315,'y':3430.906},{'x':4023.9375,'y':3425}]
  one_side_chamfered=[{'x':4023.9375,'y':3425},{'x':4092.8346,'y':3425},{'x':4092.8346,'y':3511.6142},{'x':4018.0315,'y':3511.6142},{'x':4018.0315,'y':3430.906},{'x':4023.9375,'y':3425}]
  one_side_23_chamfered=[{'x':4023.9375,'y':3425},{'x':4092.8346,'y':3425},{'x':4092.8346,'y':3515.5512},{'x':4018.0315,'y':3515.5512},{'x':4018.0315,'y':3430.906},{'x':4023.9375,'y':3425}]
  if len(outlines)!=1 or outlines[0].get('pointArr') not in (old_rectangle,old_chamfered,portrait_rectangle,portrait_chamfered,compact_rectangle,compact_chamfered,release_rectangle,release_chamfered,candidate_rectangle,candidate_chamfered,redesign_chamfered,one_side_chamfered,one_side_23_chamfered):raise RuntimeError('V2 outline does not match a recognized canonical extent')
  ...
  outline['pointArr']=one_side_23_chamfered
  ...
  for alias in TPS:
   ...
   for text in fp.get('TEXT',{}).values():text['display']='none'
   pads=fp.get('PAD',{})
  ...
  for alias in BOTTOM_PARTS:
   fp=final['FOOTPRINT'][STATE['ids'][alias]];normalize_bottom(fp)
   # These parts are physically hidden beneath the assembled ESP module.
   # Keep reference identity in BOM/PnP, but omit unusable overlapping silk.
   for text in fp.get('TEXT',{}).values():text['display']='none'
  ...
  print(json.dumps({'placed':len(POSITIONS),'bottom_pogo_pads':len(TPS),'bottom_jlc_smt':len(BOTTOM_PARTS),'top_user_smt':['U_ESP'],'board_mm':[19,23],'input_pigtail_pads':inp,'led_pigtail_pads':actual,'cable_row_y':3507.6772,'antenna_overhang_mm':5.05},indent=2))
 finally:c.close()
