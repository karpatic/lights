#!/usr/bin/env python3
"""Build the clean-sheet V2 embedded-antenna LED controller schematic."""
...
PARTS=[
 ...
 ('J_5V_IN','584e69781c5242f391e8b163168a359e','B3P-VH (LF)(SN)','regulated two-wire 5V input pigtail',100,-520),
 ('D_VBUS','c3899403907b4d44acca211d57112631','TPD1E10B06DYAR','TPD1E10B06 input ESD',270,-500),
...
def enforce_expected_schlibs(cdp,ids):
 expected={'frame_lib_1',*ids.values()}
 src=base.source(cdp)
 stale=sorted(g for g in src.get('schlib',{}) if g not in expected)
 if stale:
  base.delete_ids(cdp,stale)
 src=base.source(cdp)
 actual=set(src.get('schlib',{}))
 if actual!=expected:
  raise RuntimeError('unexpected schematic library set after duplicate cleanup: missing='+json.dumps(sorted(expected-actual))+' extra='+json.dumps(sorted(actual-expected)))
 return src
...
def main():
 ...
 try:
  ...
  final=enforce_expected_schlibs(cdp,ids);esp=final['schlib'][ids['U_ESP']]
